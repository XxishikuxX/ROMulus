...
async function downloadCover(url: string, romId: string): Promise<string | null> {
  try {
    ...
    const coverPath = path.join(process.env.COVERS_PATH || '/opt/emuverse/data/covers', filename);
...
async function downloadScreenshots(urls: string[], romId: string): Promise<string[]> {
  ...
  const screenshotsDir = path.join(
    process.env.SCREENSHOTS_PATH || '/opt/emuverse/data/screenshots',
    romId
  );
