  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const destDir = isPublic 
        ? path.join(process.env.ROM_LIBRARY_PATH || '/opt/romulus/data/roms/library', systemFolder)
        : path.join(process.env.USER_ROM_PATH || '/opt/romulus/data/roms/users', req.user!.id, systemFolder);
...
router.post('/cover/:romId', async (req: AuthRequest, res) => {
  try {
    ...
    const coversDir = process.env.COVERS_PATH || '/opt/romulus/data/covers';
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const biosDir = path.join(
        process.env.BIOS_PATH || '/opt/romulus/data/bios',
        system
      );
...
router.post('/avatar', async (req: AuthRequest, res) => {
  try {
    ...
    const avatarsDir = path.join(
      process.env.DATA_DIR || '/opt/romulus/data',
      'avatars'
    );
