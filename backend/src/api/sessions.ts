...
const EMULATOR_CONFIG: Record<string, {
  ...
  PS3: {
    command: '/opt/emuverse/emulators/rpcs3/rpcs3.AppImage',
  ...
  PS1: {
    command: '/opt/emuverse/emulators/duckstation/DuckStation.AppImage',
  ...
  VITA: {
    command: '/opt/emuverse/emulators/vita3k/Vita3K',
  ...
  XBOX360: {
    command: 'wine',
    args: (romPath) => ['/opt/emuverse/emulators/xenia/xenia.exe', romPath]
  },
  WIIU: {
    command: '/opt/emuverse/emulators/cemu/Cemu',
  ...
  N3DS: {
    command: '/opt/emuverse/emulators/citra/citra-qt',
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const savePath = path.join(
        process.env.SAVES_PATH || '/opt/emuverse/data/saves',
        req.user!.id,
        session.romId
      );
