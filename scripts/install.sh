...
# EmuVerse Installation Script for Ubuntu Server 24.04 LTS
...
setup_database() {
    ...
    sudo -u postgres psql -c "CREATE USER emuverse WITH PASSWORD '${DB_PASSWORD}';" || true
    sudo -u postgres psql -c "CREATE DATABASE emuverse OWNER emuverse;" || true
    sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE emuverse TO emuverse;" || true
    
    # Save credentials
    echo "DATABASE_URL=postgresql://emuverse:${DB_PASSWORD}@localhost:5432/emuverse" > "${CONFIG_DIR}/database.env"
...
configure_nginx() {
    ...
    cat > /etc/nginx/sites-available/emuverse << 'NGINX_CONF'
upstream emuverse_backend {
    server 127.0.0.1:4000;
    keepalive 64;
}

upstream emuverse_streaming {
...
server {
    ...
    # Frontend
    location / {
        root /opt/emuverse/app/frontend/dist;
    ...
    # API
    location /api {
        proxy_pass http://emuverse_backend;
    ...
    location /stream {
        proxy_pass http://emuverse_streaming;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
    ...
    # ROM uploads
    location /upload {
        proxy_pass http://emuverse_backend/api/upload;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
...
NGINX_CONF
    ...
    ln -sf /etc/nginx/sites-available/emuverse /etc/nginx/sites-enabled/
...
create_services() {
    ...
    # Backend service
    cat > /etc/systemd/system/emuverse-backend.service << 'SERVICE'
[Unit]
Description=EmuVerse Backend API
After=network.target postgresql.service redis.service

[Service]
Type=simple
User=www-data
WorkingDirectory=/opt/emuverse/app/backend
ExecStart=/usr/bin/node dist/server.js
Restart=on-failure
RestartSec=10
StandardOutput=append:/opt/emuverse/logs/backend.log
StandardError=append:/opt/emuverse/logs/backend-error.log
...
SERVICE
    ...
    # Streaming service
    cat > /etc/systemd/system/emuverse-streaming.service << 'SERVICE'
[Unit]
Description=EmuVerse Game Streaming Server
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/opt/emuverse/app/streaming
ExecStart=/usr/bin/node dist/server.js
Restart=on-failure
RestartSec=10
StandardOutput=append:/opt/emuverse/logs/streaming.log
StandardError=append:/opt/emuverse/logs/streaming-error.log
Environment=NODE_ENV=production
...
SERVICE
    ...
    cat > /etc/systemd/system/emuverse-xvfb.service << 'SERVICE'
[Unit]
Description=EmuVerse Virtual Framebuffer
After=network.target
...
ROM_LIBRARY_PATH=/opt/emuverse/data/roms/library
USER_ROM_PATH=/opt/emuverse/data/roms/users
BIOS_PATH=/opt/emuverse/data/bios
SAVES_PATH=/opt/emuverse/data/saves
COVERS_PATH=/opt/emuverse/data/covers
SCREENSHOTS_PATH=/opt/emuverse/data/screenshots

# Emulator Paths
EMULATOR_PATH=/opt/emuverse/emulators
...
print_final_instructions() {
    echo ""
    echo -e "${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║           EmuVerse Installation Complete!                    ║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo "   sudo systemctl start emuverse-xvfb"
    echo "   sudo systemctl start emuverse-backend"
    echo "   sudo systemctl start emuverse-streaming"
    echo ""
...
main() {
    ...
    echo ""
    echo -e "${YELLOW}This script will install EmuVerse and its dependencies.${NC}"
